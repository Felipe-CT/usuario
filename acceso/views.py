...
class LoginView(View):
    ...
    def get(self, request):
        ...
        return render(request, 'acceso/index.html', contexto)
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = Usuario.objects.filter(Q(email=form.cleaned_data['username'])).first()
            if user:
                form_password = form.cleaned_data['password']
                if bcrypt.checkpw(form_password.encode(), user.password.encode()):
                    
                    request.session['usuario'] = { 'nombre' : user.nombre, 'email' : user.email, 'id': user.id }
                    return redirect('/')
                else:
